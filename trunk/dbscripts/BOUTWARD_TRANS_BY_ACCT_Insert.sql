...
/****** Object:  StoredProcedure [dbo].[BOUTWARD_TRANS_BY_ACCT_Insert]    Script Date: 28/10/2014 2:13:35 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[BOUTWARD_TRANS_BY_ACCT_Insert]( @ID nvarchar(20),@Status nvarchar(20),@ProductID nvarchar(20),@ProductName nvarchar(250),@BenComID nvarchar(50)
      ...
      ,@SaveTemplate nvarchar(50),@VATSerial nvarchar(50),@ChargeAmtLCY decimal(18,5),@ChargeVATAmt decimal(18,5))
AS
...
UPDATE DBO.BOUTWARD_TRANS_BY_ACCT SET [ID] = @ID,[Status] = @Status,[ProductID] = @ProductID,[ProductName] = @ProductName,[BenComID] = @BenComID
      ...
      ,[ChargeAmtLCY] = @ChargeAmtLCY,[ChargeVATAmt] = @ChargeVATAmt where [ID] = @ID
ELSE 
INSERT INTO DBO.BOUTWARD_TRANS_BY_ACCT([ID],[Status],[ProductID],[ProductName],[BenComID],[BenComName],[Currency],[DebitAcctID],[DebitAcctName],[DebitAmount]
      ...
      ,@BankName ,@PayNumber ,@TellerID ,@Narrative ,@Narrative2 ,@WaiveCharge ,@SaveTemplate ,@VATSerial ,@ChargeAmtLCY ,@ChargeVATAmt)
GO
