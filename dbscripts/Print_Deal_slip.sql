...
/****** Object:  StoredProcedure [dbo].[Print_Deal_slip]    Script Date: 16/11/2014 10:31:14 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Print_Deal_slip](@type nvarchar(50),@typeProduct nvarchar(50), @RefID nvarchar(50))
AS
DECLARE @NUM decimal
...
IF(@type = 'Trans_By_Acct')
	...
	IF (@typeProduct = 'CITAD')
		BEGIN
			...
			, [Narrative]+ ', '+[Narrative2]   as Narrative  
...
ELSE IF(@type = 'Cheque' AND @typeProduct = 'withdrwal')
	BEGIN
		...
		,[ChequeNo], [ID] AS RefID,  day([CreatedDate]) as day, month([CreatedDate]) as month, year([CreatedDate]) as year,
		right('0'+ convert(nvarchar,day(getdate())),2) +  right('0'+ convert(nvarchar,month(getdate())),2) + right('0'+ convert(nvarchar,year(getdate())),4) + ' .' + right('00000'+ convert(nvarchar,@NUM),6) as CT
...
ELSE IF(@type = 'Transfer' AND @typeProduct = 'Withdrawal')
	BEGIN
		...
		SELECT convert(nvarchar,convert(money,[DebitAmount]) ,1) +'  '+ [DebitCurrency] as Amt_cur_Debit,
